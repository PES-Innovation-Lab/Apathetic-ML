...
@app.route('/api/master/gs/start', methods = ['GET'])
def start():
    ...
    model = GridSearch(model_func , feed_list_model , n_users = 2)
    
    a=time.time()

    print(model.fit(X_train_flat , y_train_oh , X_test_flat , y_test , batch_size = 100 , n_epochs = 10))
    print("EXEC TIME", time.time()-a)
