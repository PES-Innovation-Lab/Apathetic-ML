...
class KCluster:
    ...
    def fit_cluster(self,dataset,k=5,n_iters = 10):
        ...
        self.users = [User(ip=iplist[user],index=user,dataset = self.dataset) for user in range(self.n_users)]
...
class User:
    def __init__(self,index,ip,dataset):
        sesh=get_session()
        global subworkerlist
        self.ip=ip
        url = self.ip+'/api/worker/km/userinit/'+subworkerlist[index]
...
def start(workers):
    global cluster
    global iplist,s,dataset
    iplist = [s+str(i)+':5000' for i in range(0,int(workers))]
    print(iplist)
    with open('out','a') as stout:
        print("Starting",file=stout)
    a=time.time()
